...
import { Select } from "@/app/core/components/ui";
...
export default function ExchangeRatesPage() {
  ...
  async function refresh() {
    ...
    try {
      const ccys = await currenciesApi.listCurrencies(token);
      setCurrencies(ccys);
  ...
  async function saveEdit() {
    ...
    setBusy(true);
    try {
      await currenciesApi.updateCurrency(token, editRate.currency.id, {
  ...
  async function deleteRate(c: TenantCurrency) {
    ...
    setBusy(true);
    try {
      await currenciesApi.deleteCurrency(token, c.id);
      await refresh();
    } catch (e) {
      ...
      <div className="border border-[var(--color-border)] rounded-lg p-4 mb-4 bg-[var(--color-bg-soft)]">
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <div>
            ...
            <Select className="min-w-[90px]" value={previewFrom} onChange={(e) => setPreviewFrom(e.target.value)}>
          ...
          <div>
            ...
            <Select className="min-w-[90px]" value={previewTo} onChange={(e) => setPreviewTo(e.target.value)}>
              <option value="">Select…</option>
              {currencies.map((c) => <option key={c.id} value={c.code}>{c.code}</option>)}
            </Select>
          </div>
