...
import { settingsApi, type QueueScheduleMode, type TenantSettings } from "@/apps/settings/services/settings.api";
import { getToken } from "@/apps/auth/utils/token";
import {
  DEFAULT_PREFERENCES, fetchPreferences, type Preferences,
} from "@/apps/settings/utils/preferences";
import { Card, Field, FieldGrid, PageHeader } from "@/app/core/components/ui";
...
export default function PreferencesPage() {
  ...
  useEffect(() => {
    ...
    settingsApi.getTenantSettings(token).then((s) => {
  ...
  async function saveStrategy(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const raw = await settingsApi.putTenantSettings(token, {
  ...
  async function saveDisplay(e: React.FormEvent) {
    ...
    if (!token) return;
    try {
      const res = await settingsApi.putUserPreferences(token, {
