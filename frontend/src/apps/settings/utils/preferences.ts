...
import { settingsApi, type UserPreferences, type UserPreferencesUpdate } from "@/apps/settings/services/settings.api";
...
export function usePreferences(): [Preferences, (patch: Partial<Preferences>) => Promise<void>] {
  ...
  useEffect(() => {
    ...
    if (token) {
      settingsApi.getUserPreferences(token)
  ...
  const save = useCallback(async (patch: Partial<Preferences>) => {
    const token = getToken();
    if (!token) return;
    const res = await settingsApi.putUserPreferences(token, toApi(patch));
...
export async function fetchPreferences(): Promise<Preferences> {
  ...
  try {
    return fromApi(await settingsApi.getUserPreferences(token));
  } catch {
