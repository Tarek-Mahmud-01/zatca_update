...
import { HeaderActions } from "@/app/core/components/HeaderActions";
import { NotificationFeed } from "@/app/core/components/NotificationFeed";
import { Toaster } from "@/app/core/components/Toaster";
import { SessionProvider } from "@/apps/auth/store/SessionProvider";
import { handleAuthExpired, isTokenLive, subscribeAuthExpired } from "@/apps/auth/utils/token";
...
const NAV: ReadonlyArray<{ href: string; label: string; group?: string }> = [
  ...
  { href: "/dashboard/account",                label: "My account",       group: "Settings" },
  { href: "/dashboard/finance",                label: "Finance",          group: "Settings" },
  { href: "/dashboard/design",                 label: "Design system",    group: "Developer" },
];

/**
 * Route-level auth gate — fast, no network round-trip. Trusts the cookie JWT's
 * `exp` claim locally; bounces to /login if missing/expired. SSE + the 401
 * handler catch server-side revocation.
 */
